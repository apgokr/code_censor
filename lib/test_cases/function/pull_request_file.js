...
var codeCensorPullRequestFile = function(){
        ...
        if (applicableFileTypes.find(function(item){
            ...
            prFile.lines.forEach(function (prLine, index) {
                if (typeof prFile.lines[index + 1] !== 'undefined') {
                    const pattern = new RegExp("\\/\\/|\\/\\*\\*|use\s+");
    ...
    function pullRequestFileTwigContentField(prFile) {
        ...
        if (prFile.type === 'html.twig') {
            prFile.lines.forEach(function (prLine, index) {
                ...
                if (pattern.test(prLine.content) && prLine.content.indexOf('*') < 0) {
                    ...
                    for (let i = index - 1; i >= 0; i--) {
                        if (prFile.lines[i] instanceof PrLine && previousPrLineNumber - 1 === prFile.lines[i].number) {
                            const regex = new RegExp("\\{\\%\\s*if.*" + contentField);
            ...
            prFile.lines.forEach(function (prLine, index) {
                ...
                if (renderArrayRegex.test(prLine.content)) {
                    ...
                    do {
                        ...
                        if (targetPrLine.content.indexOf('form') >= 0 || targetPrLine.content.indexOf('Form') >= 0) {
                            cacheNeeded = false;
                        }
                    ...
                    if (targetPrLine instanceof PrLine && targetPrLine.content.indexOf('form') >= 0 || targetPrLine.content.indexOf('Form') >= 0) {
                        cacheNeeded = false;
                    }
                ...
                if (currentPrLine instanceof PrLine) {
                    ...
                    if (pattern.test(content)) {
                        form_keywords.forEach(function (word, index) {
                            if (content.includes(word) && prevLineNumber !== currentPrLine.number) {
                                prevLineNumber = currentPrLine.number;
                                targetPrLine = currentPrLine;
                                if (!content.includes("Drupal.checkPlain")) {
                                    assigned_variables.push({
                                        'key': targetPrLine,
                                        'value': content.split("=")[0].trim().split(" ").pop()
                                    });
                                }
                            }
                        });
                    ...
                    if ((assigned_variables).length > 0) {
                        if (content.includes("Drupal.checkPlain")) {
                            assigned_variables.forEach(function (assigned_variable, index) {
                                if ((new RegExp("\\b" + assigned_variable.value + "\\b")).test(content)) {
                                    let delindex = assigned_variables.indexOf(assigned_variable);
                                    if (delindex > -1) {
                                        assigned_variables.splice(delindex, 1)
                                    }
                                }
                            });
                        }
    ...
    function pullRequestFileCheckSubscriberServiceTags(prFile) {
        ...
        if (prFile.type === 'services.yml') {
            ...
            prFile.lines.forEach(function (currentPrLine) {
                if (currentPrLine instanceof PrLine) {
                    const pattern = new RegExp("\\\\(EventSubscriber|Routing|RouteSubscriber)");
                    let content = currentPrLine.content;
                    if (is_pattern_present) {
                        if (content && content.indexOf('class') < 0) {
                            if (content.includes('tags:')) {
                                is_pattern_present = 0;
                            }
                        }
                        else {
                            if (is_pattern_present) {
    ...
    function pullRequestFileCheckInternalUrls(prFile) {
        ...
        if (prFile.type === 'js') {
            prFile.lines.forEach(function (currentPrLine) {
                if (currentPrLine instanceof PrLine) {
                    const pattern = new RegExp("(href=|)(http:\\/\\/|https:\\/\\/|)[a-z\\-0-9.]+\\/");
